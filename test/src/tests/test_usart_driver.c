...
void test_usart_get_byte(void)
{
  ...
  usart2_isr();
  char cL_rec = '\0';
  usart_get_byte(&objS_uart2, (uint8_t*)&cL_rec, 10);
  ...
  for (size_t i = 0; i < ARRAY_SIZE(u8PL_data); ++i)
  {
  ...
  {
    uint8_t u8L_byte = 0;
    TEST_ASSERT(usart_get_byte(&objS_uart2, &u8L_byte, 1));
    u8PL_buff[i] = u8L_byte;
  }
